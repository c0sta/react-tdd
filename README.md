...
No React, por exemplo, a maior parte dos componentes são puramente funções, que recebem uma `props` de _input_ e retornam sempre o mesmo elemento como _output_.
...
```jsx=
const url = '/greeting';
const { getByText, getByTestId, getByLabelText } = render(<Fetch url={url} />);
```
